...
import { Send, Sparkles, User, Bot, Loader2, ChevronLeft, Brain, Terminal, Target } from 'lucide-react';
...
const Tutor: React.FC = () => {
  ...
  const handleAutoSend = async (text: string) => {
    ...
    setIsLoading(true);
    const response = await getTutorResponse([userMessage], "Konteks: Diskusi Profesional Ahli Strategi");
    setMessages(prev => [...prev, { role: 'model', text: response || 'Maaf, sistem sedang memproses logika lain.' }]);
    setIsLoading(false);
  };
  ...
  const handleSend = async () => {
    ...
    setMessages(prev => [...prev, { role: 'model', text: response || 'Maaf, sirkuit logika saya sedang mengalami kendala teknis.' }]);
    setIsLoading(false);
  };
    ...
    return text.split('\n').map((line, i) => {
      ...
      const formattedLine = parts.map((part, j) => {
        if (part.startsWith('**') && part.endsWith('**')) {
          return <strong key={j} className="font-black text-indigo-700">{part.slice(2, -2)}</strong>;
        }
        return part;
      });
      ...
      return (
        <p key={i} className={`mb-2 last:mb-0 ${isTechnical ? 'bg-indigo-50/70 px-4 py-2.5 rounded-xl border-l-4 border-indigo-500 font-mono text-indigo-900 font-bold shadow-sm my-2' : ''}`}>
          {formattedLine}
        </p>
      );
    });
    ...
    <div className="max-w-4xl mx-auto h-[calc(100vh-10rem)] flex flex-col bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl overflow-hidden">
      ...
      <div className="bg-indigo-600 p-6 text-white flex items-center justify-between shrink-0 shadow-lg relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 relative z-10">
          <button onClick={() => navigate(-1)} className="md:hidden p-2 bg-white/10 rounded-lg hover:bg-white/20 transition-colors"><ChevronLeft size={18}/></button>
          ...
          <div>
            ...
            <p className="text-[10px] text-indigo-100 uppercase tracking-widest font-black opacity-80">Professional Omni-Specialist</p>
          </div>
        </div>
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50 scroll-smooth no-scrollbar" ref={scrollRef}>
        {messages.map((msg, i) => (
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="flex gap-3 items-center bg-white p-4 rounded-2xl border border-slate-200 shadow-sm animate-pulse">
              ...
              <span className="text-xs font-black text-slate-500 italic uppercase tracking-[0.2em]">Ahli Tutor sedang merumuskan strategi...</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-6 bg-white border-t border-slate-100 shrink-0">
        <div className="flex gap-4 items-center bg-slate-50 border border-slate-200 p-2 rounded-2xl focus-within:ring-4 focus-within:ring-indigo-100 focus-within:border-indigo-500 transition-all shadow-inner">
          ...
          <button 
            ...
            className={`
              ...
              ${isLoading ? 'bg-slate-200 text-slate-400' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-200 active:scale-95'}
            `}
          >
            <Send size={18} />
          </button>
        ...
        <div className="flex justify-center gap-4 mt-4 overflow-x-auto no-scrollbar pb-1">
          {['LATIHAN', 'BERI TRIK SNBT', 'JELASKAN ULANG', 'LEVEL SULIT'].map(cmd => (
            <button 
              key={cmd}
